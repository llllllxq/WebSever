...
private:
    ...
    int m_checked_idx; // 当前正在分析的字符在读缓存区中的位置
    ...
    HTTP_CODE parse_request_content(char* text); // 请求数据
    char* get_line() {return m_read_buf + m_start_line;}
    LINE_STATUS parse_line();//解析的一行数据，判断依据\r\n    用白话就是：把一段数据分成一行一行
    ...
    bool add_status_line(int status, const char* title); // 响应行
    bool add_headers(int content_length); //响应头部，包含下面四个函数
    bool add_content_length(int content_length);
    bool add_content_type();
    bool add_linker();
    bool add_blank_line();
    bool add_content(const char* content); //响应内容
